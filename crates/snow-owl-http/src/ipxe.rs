...
pub async fn boot_menu(State(state): State<AppState>) -> Result<impl IntoResponse, StatusCode> {
    ...
    for (idx, image) in images.iter().enumerate() {
        ...
        menu.push_str(&format!("echo Booting {} ({})\n", image.name, image.image_type));
        menu.push_str(&generate_winpe_boot(server_ip, http_port, &image.id.to_string()));
        menu.push_str("\n");
    }
...
pub async fn boot_mac(
    ...
    if let Some(machine) = machine {
        ...
        state.db.create_or_update_machine(&updated_machine).await.map_err(|e| {
            tracing::error!("Failed to update machine: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;

        // Check for active deployment
        if let Some(deployment) = state.db.get_active_deployment_for_machine(machine.id).await.unwrap() {
            let image = state.db.get_image_by_id(deployment.image_id).await
                .map_err(|e| {
            ...
            script.push_str(&generate_winpe_boot(server_ip, http_port, &image.id.to_string()));
        ...
        state.db.create_or_update_machine(&new_machine).await.map_err(|e| {
